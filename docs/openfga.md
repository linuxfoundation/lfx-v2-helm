...
| `team:formation#member:owner:project:<rootProjectId>` | dev, prod (verified 2026-09-22 — the tuple reads back on the dev ROOT object, and a team member checks `writer_guard` true on a random sample of prod projects, including ones in Formation sub-stages, while a control user is refused on the same objects) | `owner` cascades to every descendant (`owner from parent`), which is how the formation team satisfies the `writer_guard` half of `lfx-v2-formation-service`'s `set_item_status` gate (`charts/lfx-v2-formation-service/templates/ruleset.yaml`). The other half is `member` on `team:formation`, checked with no resource in the object; both must pass, and that pair is what stops an assignee closing their own checklist item | Not recorded — predates this registry. Membership of `team:formation` is maintained by the IT team outside this platform (it is absent from the `sync-global-groups` group map, which covers only `lf-staff` and `lf-contractor`) |
